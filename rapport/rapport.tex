...
Sinon, c'est qu'il existe un état $y \in V_i$, $ i < k$ accessible depuis $x$ tel que tous les états sur le chemin $\pi$ entre $x$ et $y$ soient gagnants pour $\overline{\gamma}$ (car $\overline{\gamma}$ perd s'il boucle dans $V_k$). On a par hypothèse de récurrence, $winners[y] = \overline{\gamma}$. Choisissons un $y$ qui a ces propriétés et tel que $\pi$ soit le plus grand possible et sans boucle:
\begin{itemize}
...
Réciproquement, si $winners[x] = \overline{\gamma}$, montrons que $x$ est gagnant pour $\overline{\gamma}$.
